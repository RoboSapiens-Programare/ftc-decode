...
class LocalizationTest extends OpMode {
    ...
    /**
     * This updates the robot's pose estimate, the simple drive, and updates the Panels telemetry
     * with the robot's position as well as draws the robot's position.
     */
    @Override
    public void loop() {
        if (gamepad1.aWasPressed() || gamepad2.aWasPressed()) {
...
class ForwardVelocityTuner extends OpMode {
     ...
     * when the robot has run forward enough, these last velocities recorded are averaged and
     * printed.
     */
    @Override
    public void loop() {
            ...
            case WAIT_BRAKE_TIME:
                {
                    ...
                    if (follower.getVelocity().dot(new Vector(direction, follower.getHeading()))
                            <= 0) {
                        state = State.RECORD;
                    }
                    break;
        ...
        triangle =
                follower.pathBuilder()
                        ...
                        .setLinearHeadingInterpolation(
                                startPose.getHeading(), interPose.getHeading())
 ...
 * and calculates what the robot's strafeX and forwardY offsets should be. Ensure that your strafeX
 * and forwardY offsets are set to 0 before running this OpMode. After running, input the displayed
 * offsets into your localizer constants.
 *
