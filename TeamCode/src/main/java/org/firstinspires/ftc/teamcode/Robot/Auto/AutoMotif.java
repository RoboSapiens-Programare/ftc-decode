...
public class AutoMotif extends LinearOpMode {
  ...
  private final Pose scorePose =
      new Pose(
          ...
          20,
          Math.toRadians(
              115)); // Scoring Pose of our robot. It is facing the goal at a 115 degree angle.
  private final Pose PPGPose =
      new Pose(
          100, 83.5, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark.
  private final Pose PGPPose =
      new Pose(
          100, 59.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
  private final Pose GPPPose =
      new Pose(
          100, 35.5, Math.toRadians(0)); // Lowest (Third Set) of Artifacts from the Spike Mark.
  ...
  public void buildPathsAny() {
    ...
    // here you make the first path just in case you change the startPose to a pos where you cant
    // score
