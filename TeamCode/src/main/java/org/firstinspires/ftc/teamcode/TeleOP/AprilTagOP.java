  ...
  public void moveToTag() {
    ...
    if (tagFound) {
      ...
      if (Math.abs(error) > 20 * Kp) {
        ...
        double out =
            ((Kp * Math.log(Math.abs(error) + 1) * error / Math.abs(error))
                    + (Ki * integralSum)
                    + (Kd * derivative))
                * 1;
      ...
      for (AprilTagDetection tag : detections) {
        Orientation rot =
            Orientation.getOrientation(
                tag.pose.R, AxesReference.INTRINSIC, AxesOrder.YXZ, AngleUnit.DEGREES);

        telemetry.addData("Tag ID", tag.id);
        telemetry.addData("Center", "(%.2f, %.2f)", tag.center.x, tag.center.y);
        telemetry.addData("Pose (X,Y,Z)", "(%.2f, %.2f, %.2f)", tag.pose.x, tag.pose.y, tag.pose.z);
        telemetry.addData(
            "Yaw/Pitch/Roll",
            "(%.2f, %.2f, %.2f)",
            rot.firstAngle,
